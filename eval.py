...
file_gold = open(path.join("datasets", sys.argv[1]))
file_system = open(path.join("output", sys.argv[2]))
...
with open(path.join("output", path.splitext(sys.argv[2])[0])+".eval", "w") as res:

	res.write("Comparing gold file \"" + sys.argv[1] + "\" and system file \"" + sys.argv[2] + "\"")
	res.write("\n\nPrecision, recall, and F1 score:\n")
